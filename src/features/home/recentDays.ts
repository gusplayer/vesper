 ...
 */
export function recentDayCells(stats: readonly DayStat[], now: number, weeks = 4): HeatCell[] {
...
export function focusedDayCount(cells: readonly HeatCell[]): number {
  return cells.filter((cell) => cell.intensity > 0).length;
}

/** What VoiceOver reads for the grid: the count, then what a tap does. */
export function gridSummary(cells: readonly HeatCell[], t: RecentDaysStrings): string {
