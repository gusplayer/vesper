...
| 0035 | La llave: una sesión profunda que abre y cierra otra persona escaneando un código que rota cada 30 s, verificado sin red; el código es un QR dibujado como campo de puntos; la cámara entra como capacidad y revierte esa parte del 0021; quien tiene la llave lee un recibo en persona al desbloquear, no un panel remoto, y el conteo de intentos queda fuera de la v1 | aceptada |
| 0036 | La llave en una página web (fase 2 del 0035, sin implementar): la página muestra el código pero nunca guarda el secreto, el teléfono sigue verificando sin red, emparejar exige una tabla efímera y un límite de intentos que el servidor no tiene, y una llave web nunca puede ser la única llave | propuesta |
| 0037 | El código dictado: segundo transporte de la llave del 0035, un código corto (seis dígitos desde el 0038), derivación aparte con ventana de 10 a 15 min, freno de diez intentos por sesión, mínimo de 15 min para cerrar dictando, se enciende por llave y viene apagado; abre y cierra porque el teléfono bloqueado siempre teclea, y un desbloqueo dictado no tiene recibo | aceptada |
| 0038 | Los códigos se dictan en una respiración: el código dictado del 0037 pasa de ocho símbolos a **seis dígitos** con teclado numérico y cinco intentos por sesión; lo que sostiene el candado es el tope de intentos, no la longitud, y el tope es duro porque reiniciarlo exige la llave que se ataca | aceptada |
