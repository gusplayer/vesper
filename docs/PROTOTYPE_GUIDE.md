...
| `session/active` | Tema oscuro (lo pone el store). "Llevas enfocado" + `hero` con el tiempo, objeto, modo, "ver modos ›", botón "Terminar" según profundidad (suave: termina; firme: hoja "¿por qué?" + espera 15 s; profundo: no responde, el botón lo dice). Debajo, "Pausa de 15 min" (ghost; "Pausa en 12m" mientras no se habilita; no existe en profundo). Sesión sin límite: sin barra, línea "Sin límite · desde las 8:05". En pausa la misma ruta cambia a tema claro: "Pausa", reloj hacia abajo, "Vuelves a las 10:32…", primario "Volver ahora", ghost "Terminar la sesión". |
