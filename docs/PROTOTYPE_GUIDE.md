...
| `keys/show` | Una llave de cerca: nombre editable, fecha de emparejamiento, y —solo si este teléfono **es** la llave— el código rotatorio dibujado con `KeyPattern`, que se redibuja al cambiar la ventana de 30 s. Si la llave abre este teléfono no hay código. Primario "Listo" (guarda el nombre), ghost "Quitar" tras confirmación. |
| `keys/scan` | La llave que empieza. No es ruta de sesión: todavía no hay sesión. Cámara y "La sesión no termina hasta que vuelvas a escanear"; sin permiso, la pantalla lo dice y ofrece "Permitir la cámara". Al acertar, `startWithKey` y `SessionGate` lleva a la sesión. |
