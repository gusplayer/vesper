...
| 0049 | Terminar un vínculo del círculo en el servidor: `POST /link/end` (Rechazar, Quitar, Salir del círculo) y `POST /challenge/leave`; el fin se guarda por par y `/sync` lo devuelve en `ended`; mientras el servidor desplegado conteste 404, la app lo dice (enmienda 0033 y 0044) | aceptada; desplegada el 2026-09-25 |
| 0050 | Correo de recuperación con Resend (código de seis dígitos, la clave guardada cifrada con una llave del servidor fuera de la base, sin revelar si un correo existe) y un segundo dispositivo en la misma cuenta de Apple (una identidad, un dispositivo activo; "Traerlo aquí" o "Empezar aparte" según la última vez que se usó) | aceptada |
