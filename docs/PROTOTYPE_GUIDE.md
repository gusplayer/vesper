...
| `session/unlock` | La llave que termina (ADR-0035). Ruta de sesión: pantalla completa, sin gesto de volver. Título "Escanea la llave para terminar", la cámara en un `NativeHost`, y bajo ella "Solo la llave termina esta sesión" o el error del intento ("Ese no es el código de esta sesión", "Espera al siguiente código", "La llave no termina una sesión recién empezada"). Primario "Seguir enfocado". Ghost "Escribir un código" (ADR-0037): abre un `FieldRow` numérico para teclear los seis dígitos del código dictado, que exige quince minutos de sesión y se apaga tras diez intentos fallidos. Al acertar corre `InkFlood tone="paper"` y debajo queda `session/closed` con el recibo. |
| `settings/keys` | Ajustes › Llaves. Lista de llaves con su lado ("Este teléfono la muestra" / "Abre este teléfono"); vacía dice "Todavía no tienes ninguna llave". Primario "Agregar una llave", caption "Hasta 5 llaves." |
| `keys/pair` | Emparejar. Primero dos tarjetas: "Guarda la llave" / "La usa". Guardarla dibuja una vez el código de emparejamiento (`KeyPattern`) con primario "Listo". Usarla pide la cámara en su flujo, escanea, y termina pidiendo el nombre con primario "Guardar". |
| `keys/show` | Una llave de cerca: nombre editable, fecha de emparejamiento, y —solo si este teléfono **es** la llave— el código rotatorio dibujado con `KeyPattern`, que se redibuja al cambiar la ventana de 30 s. Si la llave abre este teléfono no hay código. Interruptor "Se puede dictar" (ADR-0037, apagado por defecto) y, encendido, el código dictado tras un toque deliberado. Primario "Listo" (guarda el nombre), ghost "Quitar" tras confirmación. |
| `keys/scan` | La llave que empieza. No es ruta de sesión: todavía no hay sesión. Cámara y "La sesión no termina hasta que vuelvas a escanear"; sin permiso, la pantalla lo dice y ofrece "Permitir la cámara". Ghost "Escribir un código" para empezar con un código dictado por teléfono. Al acertar, `startWithKey` y `SessionGate` lleva a la sesión. |
...
| `modes/apps` | Búsqueda, "Seleccionadas N / 50", lista con `AppIcon` + nombre + categoría + `Check shape="box"`. Botón Listo. Con `?native=1` muestra el selector real: el de Screen Time en iOS (`BlockingSelectionView`, token opaco) o la lista de apps con lanzador en Android (`AppRow` con `AppImage`); sin la capacidad, la razón y "Volver". Con `?onboarding=1` escribe en el borrador del onboarding y sigue al paso de Screen Time. |
