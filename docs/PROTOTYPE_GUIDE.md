...
| `circle/invite` | `StatCard` con tu código grande y botón primario "Compartir código" (hoja del sistema). `FieldRow` Código + "Enviar invitación" ghost; el resultado en una línea (enviada / no existe / lleno / es tu propio código). "Invitaciones pendientes" con chips Aceptar / Rechazar. "En tu círculo" (N de 12) con "Quitar" tras confirmación. Pie: "En el prototipo nadie recibe la invitación". |
